...
$$AUC(g)=Pr\{g(x) \geq g(\widetilde{x}) | y=+1,\widetilde{y}=-1\}$$
...
$$\min_{w\in R^d}\frac{1}{2}\|w\|^2_2+C \sum^{n^+}_{i=1} \sum^{n^-}_{j=1} max\{0,1-(\langle w,x^+_i\rangle-\langle w,x^-_j\rangle)\} \qquad (1)$$

- C is a positive penalty parameter.
- Rewrite (1) into a sum of losses for individual instances

$$\frac{1}{2} \|w\|_2^2 + C \sum^T_{t=1} L_t(w) \qquad (2)$$

- $L_t(w)$ is defined as

$$L_t(w) = I_{y_t = +1}h^+_t(w) + I_{y_t = -1}h^-_t(w)$$
$$h^+_t(w) = \sum^{t-1}_{t'=1}I_{y_{t'}=-1}max\{0,1-\langle w, x_t - x_{t'}\rangle\} $$
$$h^-_t(w) = \sum^{t-1}_{t'=1}I_{y_{t'}=+1}max\{0,1-\langle w, x_{t'} - x_t \rangle\} $$
...
- Caching a small number of received training examples:
  - Buffer $B^+_t$ : storing the received positive instances
  - Buffer $B^-_t$ : storing the received negative instances
